    ...
    <div className=''>
      <div className='w-3/4 mx-auto flex md:flex-row flex-col '>
        <div className='md:w-1/2 '>
          <div className='pt-10'>
            <p className='font-Montserrat uppercase md:text-5xl text-3xl font-semibold text-black pb-5'>DISCOVER INDIA</p>
            <p className='font-Montserrat uppercase  md:text-5xl text-3xl  font-semibold text-black pb-5'> IN A</p>
            <p className='font-Montserrat uppercase  md:text-5xl text-3xl  font-semibold text-black pb-5'>NEW WAY</p>
          </div>
          <p className='text-xl font-light text-black md:pr-4 pr-0 pt-5'>Embark on a journey with Trip Discover and experience India like never before. Our innovative approach offers you unique insights into the country's rich tapestry of culture, history, and breathtaking landscapes, making every adventure unforgettable.</p>
        </div>
        <div className='md:w-1/2'>
          <div className='space-y-4'>
            <div className="grid grid-cols-3 grid-row-1 space-y-8 ">
              ...
              <div className="col-start-3">
                <img src={Statue} alt="Statue of Unity" className="w-full h-full rounded-md" />
              </div>
            </div>

            <div className="grid grid-cols-3 grid-row-1 gap-4">
              <div></div>
              <div><img src={Hawamahal} alt="Hawa Mahal" className="w-full h-full rounded-md" /></div>
              <div> <img src={Baghabeach} alt="Bagha Beach" className="w-full h-full rounded-md" /></div>
            </div>

            <div className="grid grid-cols-3 grid-row-1 gap-4">
              <div><img src={Ooty} alt="Ooty" className="w-full  h-[150px] rounded-md" /></div>
              <div> <img src={Darjeeling} alt="Darjeeling" className="w-full h-[150px] rounded-md" /></div>
              <div><img src={Kerela} alt="Statue of Unity" className="w-full  h-[150px] rounded-md" /></div>
            </div>
          </div>
        </div>
      </div>
